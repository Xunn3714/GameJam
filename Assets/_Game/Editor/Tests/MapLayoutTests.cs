...
public sealed class MapLayoutTests
{
    ...
    [Test]
    public void BlockRoleValuesRemainCompatibleAfterRemovingExit()
    {
        Assert.AreEqual(0, (int)MapBlockRole.Spawn);
        Assert.AreEqual(2, (int)MapBlockRole.Forest);
        Assert.AreEqual(3, (int)MapBlockRole.Plains);
        Assert.AreEqual(4, (int)MapBlockRole.Village);
        Assert.AreEqual(5, (int)MapBlockRole.Lake);
    }

    [Test]
    public void RequiredRolesAppearAndAllCellsUseConfiguredRoles()
    {
    ...
    [Test]
    public void ConfigurableGridAssignsEveryCellWithoutBoundarySpecialCases()
    {
    ...
    public void MissingRoleDefinitionsLeaveCellsEmptyInsteadOfThrowing()
    {
        ...
        Assert.AreEqual(6, cells.Length);
        // 只有平原有定义：出生、森林、村庄格留空，两个补位格可能也是平原。
        Assert.GreaterOrEqual(cells.Count(cell => cell.Definition == null), 3);
