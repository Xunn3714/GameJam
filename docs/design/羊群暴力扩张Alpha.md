...
- 出生点是一个 18×10 的羊圈（`obstacle.pen_fence`，需要 6 只），里面固定放 5 只教程羊；草地上画教程标识（WASD 移动 / 碰羊入群 / 够 6 只按 F 撞栏）。标识目前是文字 + 键帽占位，美术出图后替换 Sprite。
...
- 大羊群成员通过共享空间网格查询局部邻居，50 只以上开始错峰重算转向；静止时最多 20%（且不超过 24 只）非领头羊在停留点附近错峰踱步。
- 羊群归群边界会沿整体移动方向拉成长椭圆，并随规模增加提高长宽差和边缘起伏，避免大群收敛成规则圆形。
- 速度跟随与朝向响应分离：轻点方向键不会把翻面波纹延迟传到外围，持续转向时成员以与中心距离无关的短延迟自然错峰翻面。
- 羊群普通贴到围栏时不触发镜头抖动；只有在围栏交互范围内按 F 主动撞击，或 E 冲刺实际命中障碍时反馈。力度不足为硬撞，达到门槛则反馈后撞开。
- 主动撞击振幅按当前相机视野等比补偿，并在每次撞击时从完整冲击峰值重新起振，避免羊群阶段放大镜头后反馈变弱。
- 左上角调试信息默认关闭，Tab 切换。

### 2026-09-05 主动动作调整

- E 改为冲刺：短按释放基础冲刺；长按时移动减速，松开后按 0.5、1.0、1.5、2.0 秒四档提高冲刺速度和距离，蓄力最多 2 秒。
- 冲刺力度为当前羊群数量乘蓄力倍率。1.0、1.5、2.0 秒三档倍率可在 `FlockActionController` 调整，当前均为 1.0。
- 冲刺连续检测 Blocking 障碍，力度达到围栏门槛时撞碎并继续前进；力度不足、碰到不可破坏物或地图边界时立刻截停。
- 原围栏交互移动到 F；只要羊群位于围栏交互范围内，即使中心静止也能撞击。
